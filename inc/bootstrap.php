...
class WPUWooImportExport {
    ...
    public function callback_datas_from_big_json($big_json, $args = array()) {
        ...
        $first_char = fgetc($file_handle);
        if ($first_char && trim($first_char) == '[') {
            while (($line = fgets($file_handle)) !== false) {
                $line_content = trim($line);
                /* Skip array start and end */
                ...
                $json_data = json_decode($line_content, true);
    ...
    public function wp_set_post_terms($post_id, $term_names, $taxonomy = 'category') {
        ...
        if (!is_array($term_names)) {
            $post_terms_separators = apply_filters('wpuwooimportexport__post_terms_separators', array(',', '|', '/', "\n", "\r"));
            $term_names = str_replace($post_terms_separators, ';', $term_names);
